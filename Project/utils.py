...
def get_test_labels(test_path):
    """Returns a list of labels in test file
...
def get_accuracy(labels, predictions):
    ...
    return correct_classified / total

import sys

def save_tweets(tweets):
    no_line = 0
    csv_file_name = "data/tweets.csv"
    with open(csv_file_name, 'w') as csv:
        for tweet in tweets:
            csv.write(str(no_line))
            csv.write(',')
            csv.write('not_defined')
            csv.write(',')
            csv.write(str(tweet.text.encode(sys.stdout.encoding, errors='replace')))
            csv.write('\n')
            no_line += 1
