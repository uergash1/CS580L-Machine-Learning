...
def evaluate_model(model, val_tweets, unigrams, bigrams):
    correct, total = 0, len(val_tweets)
    for val_set_X, val_set_y in extract_features(val_tweets, unigrams, bigrams, feat_type=FEAT_TYPE, test_file=False):
...
def run(freq_dist_file, bi_freq_dist_file, train_processed_file, test_processed_file, real_time_data):
    ...
    tweets = process_tweets(train_processed_file, unigrams, bigrams, test_file=False)
    if TRAIN:
    ...
    for j in range(nb_epochs):
        i = 1
        for training_set_X, training_set_y in extract_features(train_tweets, unigrams, bigrams, feat_type=FEAT_TYPE, batch_size=batch_size, test_file=False):
            ...
            # sys.stdout.flush()
            i += 1
        val_acc = evaluate_model(model, val_tweets, unigrams, bigrams)
    ...
    test_tweets = process_tweets(test_processed_file, unigrams, bigrams, test_file=True)
    n_test_batches = int(np.ceil(len(test_tweets) / float(batch_size)))
    predictions = np.array([])
    # print('Predicting batches')
    i = 1
    for test_set_X, _ in extract_features(test_tweets, unigrams, bigrams, feat_type=FEAT_TYPE, batch_size=batch_size, test_file=True):
    ...
    if real_time_data:
        ...
        labels = utils.get_test_labels(test_processed_file)
        acc = utils.get_accuracy(labels, predictions)
        print('Accuracy Logistic Regression:', acc)
        return acc


# def run_realtime_tweets(TEST_PROCESSED_FILE):
#     print()